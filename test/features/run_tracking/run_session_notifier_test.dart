...
void main() {
  ...
  test('start -> running; points accumulate distance; ticks advance elapsed',
      () async {
    final notifier = container.read(runSessionProvider.notifier);
  ...
  test('pause freezes elapsed/distance; resume skips pause movement',
      () async {
    final notifier = container.read(runSessionProvider.notifier);
    await startRunning(notifier);
    ...
    await pump();
    expect(
        container.read(runSessionProvider).distanceM, beforePause.distanceM);
    ...
    ticker.add(null);
    await pump();
    expect(container.read(runSessionProvider).elapsedS, beforePause.elapsedS + 1);
  });

  test('stop persists totals and points, stops services, returns runId',
      () async {
    final notifier = container.read(runSessionProvider.notifier);
    await startRunning(notifier);
  ...
  test('staleness: 11 ticks with no points while running -> gps lost',
      () async {
    final notifier = container.read(runSessionProvider.notifier);
    await startRunning(notifier);
  ...
  test('double-tap start: only one run row inserted, one service start',
      () async {
    final notifier = container.read(runSessionProvider.notifier);
    await notifier.startAcquiring();
    location.controller.add(pointAt(0));
  ...
  test('points arriving while stop() is in flight are ignored', () async {
    ...
    expect(container.read(runSessionProvider).phase, RunPhase.finished);
    expect(
        container.read(runSessionProvider).points.length, pointsBefore);
  ...
  test('two points with identical timestamps -> currentPace 0 ("--:--")',
      () async {
    final notifier = container.read(runSessionProvider.notifier);
    await startRunning(notifier);
    ...
    location.controller
        .add(RunPoint(lat: 3.0, lng: 101.0, timestamp: t, accuracy: 5.0));
    location.controller
        .add(RunPoint(lat: 3.0009, lng: 101.0, timestamp: t, accuracy: 5.0));
    await pump();
